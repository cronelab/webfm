...
export class Dataset {
  ...
  _initialize(data) {
    ...
    return this._validate()
      ...
      .then(function () {
        return dataset._updateDisplayData().then(function () {
  ...
  _validate() {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (metadata['_import'] != undefined) {
        reject(
          'Loaded dataset has an "_import" metadata field, and hence is not complete.'
        )
        return
      }
      ...
      if (metadata['_export'] != undefined) {
        reject(
          'Loaded dataset has an "_export" metadata field, and hence is not complete.'
        )
        return
      }
          ...
          if (contents.values !== undefined) {
            ...
            if (contents.stats !== undefined) {
              if (contents.stats.estimators !== undefined) {
                ...
                reject(
                  'Loaded dataset has no information about recording channels.'
                )
                return
              }
            } else {
      ...
      if (
        contents.values === undefined &&
        contents.stats === undefined &&
        contents.trials === undefined
      ) {
  ...
  _setupChannelStats() {
    ...
    if (this.contents.trials !== undefined) {
      ...
      channels.forEach(function (ch) {
        var newChannel = new this.ChannelStat(newChannelOpts)
  ...
  _windowInSamples(windowInSeconds) {
    ...
    var windowFloat = {
      start:
        ((windowInSeconds.start - dataWindow.start) / totalTime) * totalSamples,
      end:
        ((windowInSeconds.end - dataWindow.start) / totalTime) * totalSamples,
    }
      ...
      return forEachAsync(
        ...
        function (ch) {
          dataset.displayData[ch] =
            dataset._channelStats[ch].fdrCorrectedValues(0.05)
        },
        {
          batchSize: 5,
        }
  ...
  dataForTime(time) {
    ...
    return Object.keys(this.displayData).reduce(function (obj, ch) {
      obj[ch] =
        (1.0 - timeFrac) * dataset.displayData[ch][timeIndex] +
        timeFrac * dataset.displayData[ch][timeIndex + 1]
      return obj
    }, {})
  }
