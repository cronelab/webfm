...
fmonline.OnlineDataSource.prototype = {
  ...
  _bciDidConnect: function (event) {
    ...
    console.log('right before this.ensureRunning()')
    ...
    return new Promise(function (resolve, reject) {
      ...
      var checkRunning = function () {
        if (!manager._bciConnection.connected()) {
          ...
          if (manager.config.debug) {
            console.log(
              'Could not check whether BCI2K is running: Not connected to BCI2K.'
            )
          }
          // Try again later
        ...
        manager._bciConnection.execute('Get System State').then(result => {
          if (manager.config.debug) {
          ...
          if (result == 'Running') {
            executed = true
            resolve(true)
          } else if (result == 'Resting' && executed == true) {
            executed = false
            manager._connectToData()
          }
...
fmonline.DataFormatter.prototype = {
    ...
    this._sourceConnection.onStateFormat = function (format) {
      ...
      formatter._timingState.map(x => {
        if (format[x] === undefined) {
          console.log(
            'WARNING: Desired timing state ' +
              formatter._timingState +
              ' was not detected in the state format for Source.'
          )
        }
      })
    }
  ...
  _setupBuffers: function () {
    ...
    var blockLengthSeconds =
      this.sourceProperties.numelements * this.sourceProperties.elementunit.gain
  ...
  _sendTrial: function () {
    ...
    var trialData = this.featureBuffer.map(function (dv) {
      return dv.slice(
        dv.length - deltaBlocks - formatter._trialBlocks,
  ...
  _formatFeatureData: function (trialData) {
    ...
    return this.featureChannels.reduce(function (obj, ch, i) {
      ...
      if (formatter.normalize && formatter.featureBlockNumber >= 2)
        obj[ch] =
          (trialData[i] - formatter._featureMean[i]) /
