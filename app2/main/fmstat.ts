...
export class ChannelStat {
  ...
  pointwiseCorrectedValues(alpha, bothWays) {
    ...
    var threshold = ppfn(1 - alpha / (twoTailed ? 2 : 1), 0.0, 1.0)
  ...
  baselineComparisonPValues() {
    ...
    return tValues.map(function (t) {
      ...
      return 2.0 * (1.0 - stat.cdfn(Math.abs(t), 0.0, 1.0))
    })
  }
  ...
  fdrCorrectedValues(fdr) {
    ...
    // Sort p-values
    var sortResult = stat.argsort(pValues)
