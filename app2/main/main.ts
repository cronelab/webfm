...
var updateRecordListForSubject = function (theSubject) {
  cronelib
    .promiseJSON(path.join(apiPath, 'list', theSubject))
...
var prepareSubjectDependencies = function (theSubject) {
  ...
  Promise.all([loadSubjectBrain(), loadSubjectGeometry()])
    .then(function (data) {
...
var prepareTaskDependencies = function (taskName) {
  ...
  cronelib
    .promiseJSON(path.join(configPath, 'tasks'))
...
if (document.title == 'WebFM: Live') {
  var ingestFeatureSignal = function (featureSignal) {
    ...
    if (featureSignalBufferManager.useFeatureSignalBuffer) {
      ...
      let maxVal = Object.keys(featureSignal).reduce((a, b) =>
        featureSignal[a] > featureSignal[b] ? a : b
      )
      ...
      Object.keys(_buffered).forEach(key => {
        //@ts-ignore
        _buffered[key] =
          _buffered[key] / (_buffered[maxVal] + sliderVal.value * 0.01)
      })
  ...
  var bufferFeatureSignal = function (featureSignal) {
    ...
    var runningAverage = function (
      ...
      return (
        (oldAverage * numOldMeasurements + newMeasurement) /
        (numOldMeasurements + 1)
      )
    }
    var runningStandardDeviation = function (
      newMeasurement,
      numOldMeasurements,
      oldAvg,
      newAvg,
      ...
      return Math.sqrt(
        (oldStdev ^
          (2 * (numOldMeasurements - 1) +
            (newMeasurement - oldAvg) * (newMeasurement - newAvg))) /
          (numNewMeasurements - 1)
      )
    }
    ...
    if (featureSignalBufferManager.featureSignalBufferLength === Infinity) {
      ...
      for (var c = 0; c < chans.length; c++) {
        if (
          featureSignalBufferManager.featureSignalBuffer[chans[c]].length == 2
        ) {
          featureSignalBufferManager.featureSignalBuffer[chans[c]].shift()
        }
        featureSignalBufferManager.featureSignalBuffer[chans[c]].push(
          featureSignal[chans[c]]
        )
      }
    } else {
      // Add to buffer
      for (var c = 0; c < chans.length; c++) {
        if (
          featureSignalBufferManager.featureSignalBuffer[chans[c]].length ==
    ...
    for (var c = 0; c < chans.length; c++) {
      if (
        featureSignalBufferManager.featureSignalBuffer[chans[c]].length == 1
      ) {
        ...
        if (featureSignalBufferManager.featureSignalBufferLength === Infinity) {
          ...
          featureSignalBufferManager.runningAverage[chans[c]] = runningAverage(
            featureSignalBufferManager.featureSignalBuffer[chans[c]][1],
            featureSignalBufferManager.numBufferPoints - 1,
            featureSignalBufferManager.runningAverage[chans[c]]
          )
          // Calculate running standard deviation
...
var ingestTrial = function (trialData) {
  ...
  dataset.ingest(trialData, currStimCode).then(function () {
    if (document.title.indexOf('WebFM: Map') >= 0) {
      updateDataDisplay()
    }
...
var updateBaselineWindow = function (newWindow) {
  ...
  dataset.updateBaselineWindow(newWindow).then(function () {
    if (document.title.indexOf('WebFM: Map') >= 0) {
      updateDataDisplay()
    }
    uiManager.hideIcon('working')
