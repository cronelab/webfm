...
import {
	...
	Line, LineBasicMaterial, SphereGeometry, MeshBasicMaterial, Geometry
...
const Brain_3 = (props) => {
	...
	useEffect(() => {
		...
		(async () => {
			...
			let camera = new PerspectiveCamera(30, 960 / 720, .1, 5000);
	...
	useEffect(() => {
		...
		if (recordType == 'HG' && threeDCoords) {
			...
			document.getElementById('dataTimer').onclick = async () => {
				...
				for (let i = 350; i < firstHorizon.offsetWidth; i++) {
					...
					lineAndDotUpdate(i);
					//@ts-ignore
					document.getElementById('dataTimer').innerHTML = `${times[Math.floor((i / firstHorizon.offsetWidth) * times.length)].toFixed(3)}s`
				}
			}
		...
		if (recordType == 'EP' && threeDCoords) {
			let recordName = document.getElementById("navbar_subject").innerText.split(":")[1].split("_")
			...
			zScores.forEach((elec, i) => {
				...
				electrodes.traverse(electrode => {
					if (electrode.name == elec.name) {
						...
						electrode.material.color.setHSL(zScoreVals[i] / Math.max(...zScoreVals), 1, .5)

						let geometry = new Geometry();
						geometry.vertices.push(new Vector3(0, 0, 0));

						let x = electrode.getWorldPosition().x - stimElec1.getWorldPosition().x
						let y = electrode.getWorldPosition().y - stimElec1.getWorldPosition().y
						let z = electrode.getWorldPosition().z - stimElec1.getWorldPosition().z
						geometry.vertices.push(new Vector3(x, y, z));
						let material = new LineBasicMaterial({ color: 0x0000ff });
						// material.color.setHSL(1, 1, .5)
						material.color.setHSL(zScoreVals[i] / Math.max(...zScoreVals), 1, .5)
	...
	const dotUpdator = (position) => {
		...
		[...nodes].map((node, _i) => {
			...
			// @ts-ignore
			document.getElementById('dataTimer').innerHTML = times[zedIndex].toFixed(3)
			...
			if (electrode) {
				...
				if (rasterValue != 0) {
					electrode.scale.set(Math.abs(rasterValue) + 1, Math.abs(rasterValue) + 1, Math.abs(rasterValue) + 1)
			...
			<Container fluid>
				<Row>
					<Col sm={10}>
						...
						<Row>
							<Col>
								<Slider axis="x" x={gyriState} onChange={({ x }) => setGyriState(state => (x))} />   Gyri
							</Col>
						</Row>
						<Row>
							<Col>
								<Slider axis="x" x={wmState} onChange={({ x }) => setWmState(state => (x))} />   White Matter
							</Col>
						</Row>
						<Row>
							<Col>
								<Slider axis="x" x={subStructState} onChange={({ x }) => setSubStructState(state => (x))} />   Subcortical structures
							</Col>
						</Row>
					</Col>
				</Row>
