...
export default function CCSR(props) {
	...
	useEffect(() => {
		...
		(async () => {
			...
			//@ts-ignore
			Object.keys(respData.lgData.times).forEach(channel => {
			...
			let lgTimes = respData.lgData
			let hgTimes = respData.lgData.times
			setData(lgTimes)
		})()
	}, [])
	...
	useEffect(() => {
		if (data) {
			var x = scaleLinear()
				.domain([0, data.times[Object.keys(data.times)[0]].length])
				.range([0, width])

			setZeroMark(x(0))

			let responseGeometry = Object.keys(data.times).map(electrode => {
				let y = scaleLinear()
					.domain(extent(data.times[electrode]))
					.range([0, 40]);
	...
	return (
		<div id="fm">
			{data && Object.keys(data.times).map((electrode, i) => {
				return (
					// <OverlayTrigger
					// 	placement="auto"
					// 	overlay={
