...
export default function Cortstim() {
  ...
  const [uniqueElectrodePairs, setUniqueElectrodePairs] = useState([]);
  const [uniqueElectrodes, setUniqueElectrodes] = useState([]);
  const [colors, setColors] = useState([]);
  const [uniqueTasks, setUniqueTasks] = useState([]);

  const { setNewSubject, subject, imgLoaded } = useContext(Context);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      let electrodes = results.map((result) => result.electrodes);
      let color = results.map((result) => result.color);
      setColors(color);
      let tasks = results.map((result) => result.result);
      setUniqueElectrodes(
        electrodes
          .flatMap((elec) => elec.split("_"))
          .filter((v, i, a) => a.indexOf(v) === i)
      );
      setUniqueTasks(tasks.filter((v, i, a) => a.indexOf(v) === i));
      setUniqueElectrodePairs(
        electrodes.filter((v, i, a) => a.indexOf(v) === i)
      );
  ...
  useEffect(() => {
    if (uniqueElectrodes.length > 0) {
      uniqueElectrodes.forEach((elec) => {
        highlightElectrodes(elec, "red");
      });
      uniqueElectrodePairs.forEach((elecPair, i) => {


        createLine(elecPair.split("_")[0], elecPair.split("_")[1], colors[i]);
      });
    }
  }, [imgLoaded]);
  ...
  const disableHemisphere = (hem) => {
    ...
    wm.traverse((child) => {
      if (child.type == "Mesh") {
        if (
          child.name.startsWith(hem) ||
          child.name.startsWith(hem.toUpperCase())
        ) {
          child.visible = false;
        }
      }
      ...
      <Container fluid>
        <Row>
          <Col sm={5} style={{ padding: "0" }}>
            {subject.name ? (
              <Brain containerid="container"></Brain>
            ) : (
              <div></div>
            )}
          </Col>
          ...
          <Col>
            <Row>
              <Col>
                <ListGroup>
                  {uniqueElectrodePairs.map((elec) => {
                    return <ListGroupItem>{elec}</ListGroupItem>;
                  })}
                </ListGroup>
              </Col>
              <Col>
