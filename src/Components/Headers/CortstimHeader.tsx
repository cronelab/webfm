...
export default function CortstimHeader() {
  ...
  function parseJSONToCSVStr(jsonData) {
    ...
    jsonData.results.forEach((x) => {
      ...
      if (x.cortstimNotes) {
        csvStr += `${x.electrodes}, ${x.time}, ${x.task}, ${x.current}, ${
          x.frequency
        }, ${x.duration}, ${x.result}, ${x.cortstimNotes.replace(",", ".")}, ${
          x.color
        }, ${x.resultingArea} \n`;
      } else {
            ...
            <Table hover>
              ...
              <tbody>
                {data.results.map((result) => {
                  return (
                    <tr>
                      <th scope="row">{result.electrodes}</th>
                      <th scope="row">{result.time}</th>
                      <th scope="row">{result.task}</th>
                      <th scope="row">{result.current}</th>
                      <th scope="row">{result.frequency}</th>
                      <th scope="row">{result.duration}</th>
          ...
          <Tab eventKey="guide" title="Guide">
            <Modal.Body>
              <ListGroup>
                ...
                <ListGroup.Item>
                  If titrating, select "No task", if performing a task, select
                  "Task"
                </ListGroup.Item>
                <ListGroup.Item>
                  If performing a task, select the task from the "Task
                  description" dropdown
                </ListGroup.Item>
                <ListGroup.Item>
                  If an effect was elicited select which effect from the
                  "Effect" dropdown, else select clear
                </ListGroup.Item>
                <ListGroup.Item>
                  If the effect was a motor or sensory response, select "Motor"
                  or "Sensory" from the "Effect" dropdown, then select
                  "Left/Right", then select which area by clicking on the
                  corresponding circle on the homunculus image
                </ListGroup.Item>
                <ListGroup.Item>
                  When complete, select the "Trial Complete" button and run the
                  next stimulation
                </ListGroup.Item>
              </ListGroup>
            </Modal.Body>
