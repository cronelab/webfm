class fmdata {
            ...
            channels.forEach((ch) => {
                ...
                if (dataset.isTimeseries()) {
                    ...
                    var statValues = mean.map(
                        (d: any, i: any) =>
                            new Gaussian(mean[i], variance[i], count[i])
                    )
    ...
    private _updateContentsStats() {
        ...
        dataset.contents.stats.estimators =
            dataset.contents.stats.estimators || {}
        ...
        Object.keys(this._channelStats).forEach((ch) => {
            ...
            if (!Array.isArray(values)) {
                dataset.contents.stats.estimators.mean[ch] =
                    extractor('mean')(values)
                dataset.contents.stats.estimators.variance[ch] =
                    extractor('variance')(values)
                dataset.contents.stats.estimators.count[ch] =
                    extractor('count')(values)
                return
            }
    ...
    dataForTime(time: any) {
        ...
        let disp = Object.keys(dataset.displayData).reduce(function (
            obj: any,
            ch
        ) {
            let ins = dataset.displayData[ch]
            obj[ch] =
                (1.0 - timeFrac) * ins[timeIndex] +
                timeFrac * ins[timeIndex + 1]
            return obj
        },
        {})
...
class ChannelStat {
    ...
    baselineNormalizedValues() {
        ...
        return this.values.map((v: any) => {
            ...
            return (
                (v.mean - stat.baseline.mean) /
                Math.sqrt(stat.baseline.variance)
            )
        })
    }
