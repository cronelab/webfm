...
class FmStat extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
			// mean       = mu,
			// variance   = s2,
			// count      = n,
			// _m2        = ( this.count > 1 && this.variance !== undefined ) ? this.variance * ( this.count - 1 ) : undefined;
			// baseline       = options.baseline          || new fmstat.Gaussian(),
			// values         = options.values            || null,
			// baselineWindow = options.baselineWindow    || { start: 0, end: 10 },
			// valueTrials    = [],
		}
	}
	ingest(datum) {
