...
window.onload = async () => {
	...
	fetch(fetchRoute).then(response => response.json()).then(data => {
		dataset.get(data).then(() => {
			...
			(<HTMLInputElement>document.getElementById('fm-option-save-name')).value = dataset.metadata.setting.task;
...
document.getElementById('playButton').onclick = async e => {
	for (let i = 0; i < dataset.contents.times.length; i++) {
		...
		let result = await new Promise((resolve, reject) => {
			setTimeout(() => resolve(dataset.dataForTime(dataset.contents.times[i])), 250);
		})


		uiManager.brain.update(result);
		document.getElementsByClassName('fm-time-selected')[0].innerHTML = `${(dataset.contents.times[i] > 0 ? '+' : '')}${dataset.contents.times[i].toFixed(3)} s`;
