...
class fmdata {
		...
		if (this.contents.stats !== undefined) {
			...
			channels.forEach(ch => {
				...
				if (dataset.isTimeseries()) {
					...
					var statValues = mean.map((d: any, i: any) => new Gaussian(mean[i], variance[i], count[i]));
	...
	_updateContentsStats() {
		...
		dataset.contents.stats.baseline.mean = dataset.contents.stats.baseline.mean || {};
		dataset.contents.stats.baseline.variance = dataset.contents.stats.baseline.variance || {};
		dataset.contents.stats.baseline.count = dataset.contents.stats.baseline.count || {};

		dataset.contents.stats.estimators = dataset.contents.stats.estimators || {};
		dataset.contents.stats.estimators.mean = dataset.contents.stats.estimators.mean || {};
		dataset.contents.stats.estimators.variance = dataset.contents.stats.estimators.variance || {};
		dataset.contents.stats.estimators.count = dataset.contents.stats.estimators.count || {};
	...
	getTrialCount() {
		...
		if (stats !== undefined) {
			if (stats.distribution.toLowerCase() == "gaussian") {
				...
				return Object.keys(counts).reduce((acc, ch) => acc.concat(counts[ch]), []).reduce((a, b) => Math.min(a, b));
			}
		}
		return undefined;
	}
