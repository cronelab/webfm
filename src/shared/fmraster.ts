...
class fmraster {
  ...
  setupCharts() {
    ...
    if (!this.timeScale) {
      this.timeScale = scaleLinear().domain([0, width]).range([0, 1]);
    } else {
      if (!this.timeScale) return;
      this.timeScale.domain([0, width]);
    }
    let horizonChart1 = horizonChart();
    var horizons: any = select("#fm")
    ...
    horizons
      ...
      .classed(
        "fm-horizon-small",
        () => raster.channelHeight <= raster.channelHeightCutoff
      )
      .each(function (d: any, i: any) {
        horizonChart1
          .title(d.channel)
