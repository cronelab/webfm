...
class fmscope {
  ...
  _setupData() {
    this.data = Array.apply(null, new Array(this.windowSamples)).map(
  ...
  autoResize() {
    var width =
      parseFloat(
        getComputedStyle(
          document.getElementById("fm-scope"),
          null
        ).width.replace("px", "")
      ) -
      (this.plotMargin.left + this.plotMargin.right);
  ...
  _setupPlot() {
    ...
    this.plotXScale = scaleLinear().domain([0, this.windowSamples - 1]);
    this.plotYScale = scaleLinear().domain(this.dataExtent);
    this.plotLine = line()
  ...
  _updatePlot() {
    ...
    select(".fm-scope-axis-y").call(this.plotYAxis);
    if (!this.data) {
      return;
    }
    select(".fm-scope-line").datum(this.data).attr("d", this.plotLine);
  }
  ...
  _updateScale() {
    ...
    if (!this.targetExtent) {
      ...
      targetMin =
        this.targetExtent[0] === null ? dataMin : this.targetExtent[0];
      targetMax =
        this.targetExtent[1] === null ? dataMax : this.targetExtent[1];
    }
    this.dataExtent[0] =
      this.dataExtent[0] + this.extentSnap * (targetMin - this.dataExtent[0]);
    this.dataExtent[1] =
      this.dataExtent[1] + this.extentSnap * (targetMax - this.dataExtent[1]);
