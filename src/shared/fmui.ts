...
class fmui {
	...
	rewireButtons() {
		var manager = this;
		(<HTMLButtonElement>document.getElementsByClassName('fm-zoom-in')[0]).onclick = (event: any) => {
			event.preventDefault();
			manager.zoom(event, 1);
		};
		(<HTMLButtonElement>document.getElementsByClassName('fm-zoom-out')[0]).onclick = (event: any) => {
			event.preventDefault();
			manager.zoom(event, -1);
		};
		...
		(<HTMLButtonElement>document.getElementsByClassName('fm-show-options')[0]).onclick = event => {
			event.preventDefault();
		...
		(<HTMLButtonElement>document.getElementsByClassName('fm-save-cloud')[0]).onclick = event => {
			event.preventDefault();
	...
	rewireForms() {
		...
		$('#fm-options-modal').on('hidden.bs.modal', (event: any) => manager.optionsHidden(event));
		...
		document.getElementById('fm-option-stim-timing-state').onclick = (event) => {
			(<HTMLInputElement>document.getElementById('fm-option-stim-timing-state')).checked = true;
			(<HTMLInputElement>document.getElementById('fm-option-stim-timing-signal')).checked = false;
			updateOptions((options: any) => options.stimulus.timingStrategy = 'state');
			manager.onoptionchange('stim-timing', 'state');
		};
		document.getElementById('fm-option-stim-timing-signal').onclick = event => {
			(<HTMLInputElement>document.getElementById('fm-option-stim-timing-state')).checked = false;
			(<HTMLInputElement>document.getElementById('fm-option-stim-timing-signal')).checked = true;
			updateOptions((options: any) => options.stimulus.timingStrategy = 'signal');
			manager.onoptionchange('stim-timing', 'signal');
		};

		document.getElementById('fm-option-stim-channel').onchange = function (event: any) {
			var newValue = event.target.value;
			updateOptions((options: any) => options.stimulus.signal.channel = newValue);
			manager.onoptionchange('stim-channel', newValue);
		};
		document.getElementById('fm-option-stim-off').onchange = function (event: any) {
			var newValue = +event.target.value;
			updateOptions((options: any) => options.stimulus.signal.offValue = newValue);
			manager.onoptionchange('stim-off', newValue);
		};
		document.getElementById('fm-option-stim-on').onchange = function (event: any) {
			var newValue = +event.target.value;
			updateOptions((options: any) => options.stimulus.signal.onValue = newValue);
			manager.onoptionchange('stim-on', newValue);
		};

		document.getElementById('fm-option-stim-trial-start').onchange = (event: any) => {
			updateOptions((options: any) => options.stimulus.window.start = parseInt(event.target.value));
		};
		document.getElementById('fm-option-stim-trial-end').onchange = (event: any) => {
			updateOptions((options: any) => options.stimulus.window.end = parseInt(event.target.value));
		};

		document.getElementById('fm-option-stim-baseline-start').onchange = (event: any) => {
			updateOptions((options: any) => options.stimulus.baselineWindow.start = parseInt(event.target.value));
		};
		document.getElementById('fm-option-stim-baseline-end').onchange = (event: any) => {
			updateOptions((options: any) => options.stimulus.baselineWindow.end = parseInt(event.target.value));
		};


		document.getElementById('fm-option-stim-state').onchange = (event: any) => {
			updateOptions((options: any) => options.stimulus.state.name = event.target.value);
			manager.onoptionchange('stim-channel', event.target.value);
		};
		document.getElementById('fm-option-stim-state-off').onchange = (event: any) => {
			updateOptions((options: any) => options.stimulus.state.offValue = event.target.value);
			manager.onoptionchange('stim-off', event.target.value);
		};
		document.getElementById('fm-option-stim-state-on').onchange = (event: any) => {
			updateOptions((options: any) => options.stimulus.state.onValue = event.target.value);
			manager.onoptionchange('stim-on', event.target.value);
		};
		document.getElementById('fm-option-stim-state-exclude').onchange = (event: any) => {
			updateOptions((options: any) => options.stimulus.state.exclude = event.target.value);
		...
		document.getElementById('fm-option-scope-channel').onchange = function (event: any) {
			manager.scope.start(event.target.value);
		};

		document.getElementById('fm-option-scope-min').onchange = function (event: any) {
			manager.updateScopeMin((event.target.value == '') ? null : +event.target.value);
		};
		document.getElementById('fm-option-scope-max').onchange = function (event: any) {
			if (isNaN((event.target.value == '') ? null : +event.target.value)) return;
			manager.scope.targetExtent[1] = (event.target.value == '') ? null : +event.target.value;
		};
	}
	...
	_populateOptions(options: any) {
		// Stimulus windows
		(<HTMLInputElement>document.getElementById('fm-option-stim-trial-start')).value = options.stimulus.window.start;
		(<HTMLInputElement>document.getElementById('fm-option-stim-trial-end')).value = options.stimulus.window.end;
		(<HTMLInputElement>document.getElementById('fm-option-stim-baseline-start')).value = options.stimulus.baselineWindow.start;
		(<HTMLInputElement>document.getElementById('fm-option-stim-baseline-end')).value = options.stimulus.baselineWindow.end;
		...
		(<HTMLInputElement>document.getElementById('fm-option-stim-channel')).value = options.stimulus.signal.channel;
		this.onoptionchange('stim-channel', options.stimulus.signal.channel);
		(<HTMLInputElement>document.getElementById('fm-option-stim-off')).value = options.stimulus.signal.offValue;
		this.onoptionchange('stim-off', options.stimulus.signal.offValue);
		(<HTMLInputElement>document.getElementById('fm-option-stim-on')).value = options.stimulus.signal.onValue;
		this.onoptionchange('stim-on', options.stimulus.signal.onValue);

		(<HTMLInputElement>document.getElementById('fm-option-stim-state')).value = options.stimulus.state.name;
		(<HTMLInputElement>document.getElementById('fm-option-stim-state-off')).value = options.stimulus.state.offValue;
		(<HTMLInputElement>document.getElementById('fm-option-stim-state-on')).value = options.stimulus.state.onValue;
	...
	clearExclusion() {
		...
		[].forEach.call(document.getElementsByClassName('fm-montage-cell-isexcluded'), (a: any) => a.innerText = "No")
