...
export default class OnlineDataSource {
  ...
  connect(address: string) {
    var manager = this;
    return this._bciConnection.connect(address).then((event: any) => {
  ...
  _allPropertiesReceived() {
    ...
    let featureFreqs = this.featureProperties.elements.map((e: any) => {
      this.featureProperties.elementunit.offset +
        e * this.featureProperties.elementunit.gain;
    });
    // console.log(featureFreqs)
    this._featureKernel = featureFreqs.map(
  ...
  _setupBuffers() {
    let blockLengthSeconds =
      this.sourceProperties.numelements *
      this.sourceProperties.elementunit.gain;
    let windowLengthSeconds = this._bufferWindow.end - this._bufferWindow.start;
    let windowLengthBlocks = Math.ceil(
      windowLengthSeconds / blockLengthSeconds
    );
    this.featureBuffer = this.featureChannels.reduce(function (
      arr: any,
      ch: any,
      i: any
    ) {
    ...
    []);
    this._trialBlocks = Math.ceil(
      (this.trialWindow.end - this.trialWindow.start) / blockLengthSeconds
    );
    this._postTrialBlocks = Math.ceil(
      this.trialWindow.end / blockLengthSeconds
    );
    console.log(
      "Created feature buffer: " +
        this.featureChannels.length +
        " channels x " +
        windowLengthBlocks +
        " samples."
    );
  ...
  _processStateVector(state: any) {
    ...
    if (this._stateTiming)
      state[this._timingState].some((s: any) => this._updateTimingState(s));
  }
  ...
  _updateTimingState(newState: any) {
    ...
    if (
      newState == thresholdValue ||
      thresholdValue == "x" ||
      thresholdValue == "X" ||
      newState == parseInt(exclusionState)
    ) {
      if (newState == 0) return;
      if (this.trialEndBlockNumber) {
        console.warn(
          "WARNING Received new trial state, but already in a trial. Ignoring."
        );
        return;
      }
      this.trialEndBlockNumber =
        this.stateBlockNumber - 1 + this._postTrialBlocks;
      this.onStartTrial();
      return true;
    } else {
      return false;
  ...
  _connectToData() {
    var manager = this;
    this._bciSourceConnection
      .connect(`ws://${localStorage.getItem("source-address")}:20100`)
      .then((dataConnection: any) => {
        this._bciSourceConnection.onSignalProperties = (properties: any) => {
          ...
          if (!this._stateTiming) {
            if (manager.sourceChannels.indexOf(manager._timingChannel) < 0) {
              console.log(
                "Timing channel not detected; falling back to imprecise timing."
              );
              this._stateTiming = true;
        ...
        this._bciSourceConnection.onGenericSignal = (signal: any) => {
          ...
          if (!this._stateTiming) {
            var timingIndex = manager.formatter.sourceChannels.indexOf(
              this._timingChannel
            );
        ...
        this._bciSourceConnection.onStateFormat = (format: any) => {
          if (format[this._timingState] === undefined) {
            console.log(
              `WARNING: Desired timing state ${this._timingState} was not detected in the state format for Source.`
            );
          }
        };
      });

    this._bciFilterConnection
      .connect(`ws://${localStorage.getItem("source-address")}:20203`)
      .then((dataConnection: any) => {
        let manager = this;
  ...
  _sendTrial() {
    ...
    this.ontrial(
      this._formatData(
        "Feature",
        this.featureBuffer.map((dv: any) => {
          return dv.slice(
            dv.length - deltaBlocks - this._trialBlocks,
            dv.length - deltaBlocks
          );
        })
      )
    );
