...
app.get("/api/list", function(req, res) {
  ...
  fs.readdir(dataDir, function(err, entries) {
    if (err) {
      errOut(
        500,
        "Could not read data directory contents: " + JSON.stringify(err)
      );
      return;
    }
...
app.get("/api/list/:subject", function(req, res) {
  ...
  fs.readdir(subjectDir, function(err, entries) {
    if (err) {
      errOut(
        500,
        "Could not read data directory for subject " +
          subject +
          ": " +
          JSON.stringify(err)
      );
    ...
    getSubjectMetadata(subject, function(err, metadata) {
      ...
      var handleJSONData = function(data, cb) {
        ...
        try {
          ...
          errOut(
            400,
            "New geometry JSON could not be parsed: " + JSON.stringify(err)
          );
          return;
        }
      ...
      if (reqContentType == "multipart/form-data") {
        ...
        form.on("file", function(field, file) {
          ...
          if (fileExtension == ".json") {
            ...
            fs.readFile(file.path, function(err, data) {
              if (err) {
                errOut(
                  500,
                  "Could not process uploaded JSON file: " + JSON.stringify(err)
                );
                return;
              }
          ...
          if (fileExtension == ".csv") {
            ...
            fs.readFile(file.path, function(err, data) {
              if (err) {
                errOut(
                  500,
                  "Could not process uploaded CSV file: " + JSON.stringify(err)
                );
                return;
              }
          ...
          errOut(
            400,
            'Unsupported geometry file type: "' + fileExtension + '".'
          );
        });
      ...
      errOut(
        400,
        'Request content-type, "' + reqContentType + '", is not supported.'
      );
    });
  });
});
